...
- **Hook red-team harness** (`tests/test_hooks_redteam.py` + `tests/fixtures/hook_redteam_cases.json`): 75 cases fired as real subprocesses against both pre-tool gates in a sandboxed HOME.
...
- **Destructive-verb coverage**: hard-tier `destructive_sensitive_path`/`find_delete_sensitive_path` rules cover `rm`/`unlink`/`shred`/`rmdir`/`find -delete` against credential stores, `~/.ciel` (root, hooks, risk, grant controls), `~/.config/devin`, and protected system paths — previously `rm -rf ~/.ciel/risk` matched no rule. `shell_sensitive_path_write` also gained the destructive verbs at soft tier.
- **Policy ancestor-substitution closed**: `_candidate_policy_files` now generates candidates only under named-anchor ancestors (`.ciel`, `ciel.skill`) instead of every ancestor to `/` — a planted `~/risk/policy.json` can no longer replace the deployed policy after an unguarded delete.
