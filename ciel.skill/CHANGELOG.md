...
- **System-1 shadow tier** (`risk/SYSTEM1.md`, `risk_policy.py`): optional semantic second opinion for the pre-tool gate via the Jev protocol (`POST /v1/systemone`, typed `choice`/`noul`/`score` questions). Strictly advisory — never influences `evaluate()`; hooks fire a detached `--shadow` subprocess that logs verdicts to `~/.ciel/system1/shadow.log` for calibration data. Backends: local `laya-serve` (default `http://127.0.0.1:8765`), hosted Jev, or AutoJev — swap via `CIEL_SYSTEM1_URL`/`CIEL_SYSTEM1_KEY`; `CIEL_SYSTEM1_DISABLED=1` opts out.
