...
- **System-1 calibration harness** (`scripts/system1_eval.py`): scores the endpoint against the hook red-team corpus and writes `risk/system1_calibration.json` (confusion matrix, precision/recall/F1, confidence calibration, latency) — the committed evidence artifact required before any surface promotion.
