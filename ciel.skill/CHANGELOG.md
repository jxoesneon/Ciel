...
- **Phase-4 System-1 orchestration port** (`ciel system1 --ask|--decide`): the whole `system1.py` client in Rust — `tool_state` enrichment (now shared with `shadow.rs`), cache read/write keyed by a `json.dumps(sort_keys=True)`-byte-exact canonical serializer (sorted keys, `", "`/`": "` separators, ensure_ascii escaping incl. surrogate pairs — verified by digest-parity test), 4 MiB event-log rotation, advisory `band`, inflight-marker lifecycle with 120 s stale reaping and `MAX_INFLIGHT=2` saturation drop, detached re-exec `system1 --ask` child fed the payload on stdin with `CIEL_SYSTEM1_MARKER`. HTTP layer: minimal in-process HTTP/1.1 client for `http://` (the documented local laya-serve endpoint; verified live against the running serve), `curl` subprocess for `https://` hosted backends — TLS stays out of the binary. `system1_embed.py` remains Python (sentence-transformers) and is spawned with the identical venv-helper contract including the 60 s timeout (`wait-timeout` crate). `TestSystem1Parity` covers offline/online decide, cache-hit records, marker cleanup, and the disabled-kill-switch event shape.
- **Phase-5 distribution**: `release.yml` gained a `build-bin` matrix (linux-x86_64, linux-aarch64, darwin-x86_64, darwin-arm64) publishing `ciel-<version>-<plat>` binaries + SHA-256 sums to each GitHub release. `install.sh` §9c defaults `CIEL_RELEASE_BASE` to the release asset URL so prebuilt install works out of the box; all five hook adapters additionally resolve `~/.cargo/bin/ciel`, making `cargo install --path ciel.skill/init/ciel-rs` an effective install path. Binary acquisition order: cargo build → `CIEL_BIN_URL`/`CIEL_RELEASE_BASE` prebuilt → Python fallback — never a blocker.
- **Residual Python (need-basis)** — what stays interpreted and why: `system1_embed.py` (sentence-transformers; invoked as a venv helper by the Rust orchestrator), all `init/hooks/**/*.sh` adapters + `install.sh`/`verify.sh` (runtimes invoke shell — the adapter contract itself), `init/hooks/lib/*.py` (the fallback engine the adapters drop to when no binary resolves — kept until parity is proven everywhere), and cold operator scripts (`scripts/scan_skills.py`, `paired_eval.py`, `system1_eval.py`, `system1_review.py`, `system1_export.py`, `migrate_skill_sidecar.py`, `harmonize_skills.py`, `fix_md_lint.py`, `integrity.py`) — rare-path tooling where interpreter startup is irrelevant and Python's stdlib (sqlite3, zipfile, yaml ecosystem) does the work. Every hot-path Python body has a Rust twin behind the same stdout/state contract.
