...
Generate PNG images (Gemini or xAI Grok) and GLB 3D models (Tripo3D) from text prompts. These are paid APIs — every call costs real money. Tools live at `tools/`; run from the project root and keep runtime-loaded outputs under `assets/`.
...
Read `rembg.md`. Key rule: **never prompt for a "transparent background"** (the generator bakes a checkerboard) — prompt a solid color, then matte it out.
...
```bash
python3 tools/asset_gen.py glb  --image ref.png -o model.glb     # 30¢ default / 60¢ --quality hd
python3 tools/asset_gen.py rig  --image ref.png -o rigged.glb    # +25¢, biped only
python3 tools/asset_gen.py retarget --rigged rigged.glb \
...
```bash
_log=$(mktemp)
result=$(python3 tools/asset_gen.py image --prompt "..." -o p.png 2>"$_log") || tail -20 "$_log"
```
