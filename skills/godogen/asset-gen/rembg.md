...
Always pass `--preview` — it writes a `_qa.png` (the result composited on a contrasting color). You cannot judge transparency from a raw PNG; reading the `_qa` is the only reliable check. Delete it after inspection.
